...
function ConBadge({ amount, label }: { amount: number | null; label: string }) {
  return (
    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 rounded-xl">
      ...
      <span className="text-xs font-black text-yellow-600 dark:text-yellow-500">
        {amount !== null ? `${amount} C` : '· · ·'}
      </span>
    </div>
  );
}

// ── 정적 가이드 콘텐츠 (CON 금액 미포함) ─────────────────────────────────
...
const AI_GUIDES: Record<SubTab, { title: string; steps: { title: string; desc: string }[]; tips: string }> = {
  direct: {
    ...
    steps: [
      ...
      { title: '\'AI 문제 생성하기\' 클릭', desc: '지문 수×유형별 CON 합산 금액이 버튼 위에 차감 예정으로 표시됩니다.' },
...
export default function GuidePage() {
    ...
    fetch('/api/credits/pricing')
      ...
      .then(data => {
        ...
        items.forEach(item => {
          ...
          else if (item.feature_key.startsWith('ai_type_') && p.aiType === undefined) p.aiType = item.cost_per_use;
          else if (item.feature_key.startsWith('wb_direct_') && p.wb === undefined) p.wb = item.cost_per_use;
          // fallback 구키
  ...
  const conLabel = mainTab === 'pdf' ? '지문 1개당' : '유형 1개당';
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-10">
          ...
          {conTable.map(item => (
            <div key={item.label} className="flex items-center justify-between bg-white dark:bg-slate-800 rounded-xl px-3 py-2 border border-yellow-100 dark:border-yellow-800">
              ...
              <span className="text-xs font-black text-yellow-600 dark:text-yellow-400">
                {item.amount !== null ? `${item.amount} C` : '· · ·'}
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5 space-y-4">
        ...
        {[
          ...
          { q: 'OCR 인식이 정확하지 않아요.', a: '고해상도 이미지 사용, 페이지 평탄화, 텍스트를 수평으로 맞추면 인식 정확도가 높아집니다.' },
        ].map((faq, i) => (
