...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900">
      {/* GNB (상단 네비게이션) */}
      <nav className="flex items-center justify-between px-8 py-5 bg-white border-b border-slate-50 sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col -space-y-1">
            <span className="text-2xl font-black tracking-tighter text-slate-900">
              CLASS<span className="text-yellow-500">HUB</span>
            </span>
      ...
      <section className="relative pt-24 pb-32 overflow-hidden">
        <div className="max-w-7xl mx-auto px-8 flex flex-col items-center text-center">
          ...
          <p className="text-xl text-slate-400 font-medium leading-relaxed max-w-2xl mb-14">
            클래스허브 하나로 출결부터 성적, 수납까지.<br />
            원장님은 교육에만 집중하세요. 복잡한 관리는 저희가 합니다.
          </p>

          <div className="flex flex-col sm:flex-row gap-6">
            <Link href={isLoggedIn ? "/admin" : "/login"} className="px-12 py-5 bg-slate-900 text-white font-black rounded-full text-lg shadow-2xl shadow-slate-300 hover:bg-slate-800 transition-all hover:-translate-y-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { 
              ...
              desc: "학생별 성적 변화를 그래프로 자동 생성합니다. 상담 시 학부모님께 전문적인 데이터를 보여주세요.", 
              icon: "📊",
              bg: "bg-blue-50"
            },
            { 
              ...
              desc: "등/하원 즉시 카카오 알림톡이 발송됩니다. 학부모님이 가장 안심하고 만족하는 기능입니다.", 
              icon: "🔔",
              bg: "bg-yellow-50"
            },
            { 
              title: "자동 청구서 발행", 
              label: "Auto Billing", 
              desc: "매달 번거로운 수납 안내를 자동화하세요. 모바일 청구서 한 장으로 미납 걱정을 덜어드립니다.", 
              icon: "📄",
              bg: "bg-slate-100"
            }
          ].map((item, i) => (
            <div 
              ...
              className="group relative p-12 bg-white border-2 border-slate-200 rounded-[3.5rem] 
                         ...
                         hover:-translate-y-4 hover:shadow-[0_20px_40px_rgba(0,0,0,0.08)] 
                         hover:border-yellow-400 cursor-default"
            >
              <div className={`w-20 h-20 ${item.bg} rounded-[2rem] flex items-center justify-center text-4xl mb-8 
                              group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500`}>
                {item.icon}
              </div>
              <p className="text-yellow-600 font-black text-xs uppercase mb-2 tracking-widest opacity-80 group-hover:opacity-100">
                {item.label}
              </p>
              <h4 className="text-2xl font-black mb-4 transition-colors group-hover:text-slate-900">
                {item.title}
              </h4>
              <p className="text-slate-500 font-medium leading-relaxed transition-colors group-hover:text-slate-600">
                {item.desc}
              </p>
              <div className="absolute top-8 right-8 text-slate-200 group-hover:text-yellow-400 transition-colors text-2xl font-bold">
                ↗
              </div>
            </div>
          ))}
        ...
        <div className="max-w-7xl mx-auto bg-slate-900 rounded-[4rem] px-8 py-24 text-center relative overflow-hidden shadow-2xl shadow-slate-400">
          <div className="relative z-10">
            <h2 className="text-4xl md:text-5xl font-black text-white mb-6 leading-tight">
              성공하는 학원의 선택<br />
              <span className="text-yellow-400 italic">CLASS HUB</span>
            </h2>
            <p className="text-slate-400 font-medium mb-12">지금 가입하고 7일간 모든 기능을 무료로 체험해보세요.</p>
            <Link href={isLoggedIn ? "/admin" : "/register"} className="px-12 py-5 bg-yellow-400 text-slate-900 font-black rounded-full hover:bg-yellow-300 transition-all inline-block text-lg shadow-xl shadow-yellow-400/20">
              {isLoggedIn ? "대시보드로 바로가기" : "지금 무료로 시작하기"}
            </Link>
          </div>
          <div className="absolute -bottom-10 -right-10 text-white opacity-[0.03] text-[250px] font-black pointer-events-none">
            CH
          </div>
        </div>
      </section>
      ...
      <footer className="max-w-7xl mx-auto px-8 py-12 border-t border-slate-50">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-slate-400 text-sm font-bold">
          ...
          <p>© 2026 CLASSHUB. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
