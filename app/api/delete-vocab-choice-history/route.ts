...
export async function POST(request: Request) {
  try {
    ...
    if (!items || items.length === 0) return NextResponse.json({ error: '삭제할 항목을 찾을 수 없습니다.' }, { status: 404 });

    const paths = [
      ...items.filter(i => i.pdf_path).map(i => i.pdf_path as string),
      ...items.filter(i => i.answer_pdf_path).map(i => i.answer_pdf_path as string),
    ];
