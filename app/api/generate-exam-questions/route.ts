...
[문제 형식]
수능·평가원 스타일. question_text 고정: "다음 글의 밑줄 친 부분 중, 어법상 틀린 것은?"

[modified_passage 규칙]
- 원본 지문 전체 그대로 사용. 생략·요약·재서술 절대 금지.
- 구조적 충돌 가능 지점 5곳을 선정해 ①②③④⑤ 번호를 단어/구 바로 앞에 삽입.
- 정답 위치 1곳만 문법 오류 형태로 변형 (원문 형태 → 틀린 형태).
- 나머지 4곳은 원문 그대로, 문법적으로 완전히 맞아야 함.
- 오류는 반드시 실제 문법 규칙 위반이어야 함. awkwardness·의미 붕괴 금지.
- 정답 위치: 표면상 자연스러워 보이되, 문장 구조 분석 후에만 오류 확인 가능해야 함.

[번호 삽입 형식]
단일 단어: ①remains  /  ②whether
다중 단어: ③[which was established]  /  ④[not only reading but also analyzing]
→ 대괄호 안에 밑줄 대상 전체를 포함. 부분 발췌 금지.

[선정 가능한 구조 — 반드시 이 중에서만 선택]
학생이 아래 구조를 실제로 분석해야만 정오 판단이 가능해야 함:

1. 관계사: which/that/who/where/when/why — 선행사·격 충돌, 삽입구로 숨겨진 선행사
   좋은 예) the reason ①[why] he succeeded  /  the place ②[which] he lived in
   나쁜 예) ❌ 단순 that, ❌ 단순 which (구조 분석 불필요)

2. 병렬구조: A and/but/or B, not only A but also B, both A and B
   오류 방식: 동명사↔부정사, 형용사↔부사, 명사↔절의 category mismatch
   좋은 예) not only reading but also ①[analyze]  →  analyze가 오류 (should be analyzing)

3. 분사: 현재분사 vs 과거분사, 분사구문 논리주어
   좋은 예) ①[Compared] to mammals, birds... (논리주어 일치 판단 필요)
   오류 예) The result was ②[surprised] → should be surprising

4. 명사절 접속사: what/that/whether 선택
   whether("~인지") vs that("~라는 사실") 혼용 오류
   좋은 예) determine ①[that] → should be whether  /  research shows ②[that] (correct)

5. 수일치 (긴 삽입구 존재 시에만): 주어·동사가 관계절·삽입구·전치사구로 분리된 경우
   좋은 예) The discovery, along with several experiments, ①[suggest] → should be suggests

6. 준동사·사역동사 구조:
   - to의 품사 판단: 전치사 to+Ving vs 부정사 to+원형
     (lead to ①[reducing] ✓ / use to ②[defending] ✗ → should be defend)
   - allow/enable/require + O + to V (목적어 다음 to부정사)
   - it-가목적어: make/find/consider + it + adj + to V

7. 재귀대명사: 주어=목적어일 때 themselves/itself, 다를 때 them/it
   좋은 예) plants use ①[them] → should be themselves

[절대 밑줄 금지]
❌ 단독 전치사/관사/접속사  ❌ 단순 content word(명사·일반동사)
❌ 단순 수일치(The students ①likes)  ❌ 단순 시제 오류  ❌ 전치사 누락

[Distractor 설계 — 나머지 4개]
"틀린 것 같아 보이지만 완전히 맞는" 구조를 선택:
- lead to + Ving (전치사 to → 동명사, 정상)
- appositive 뒤 숨겨진 수일치가 맞는 경우
- 감정분사 interesting/fascinated 올바르게 사용
- nested 관계절, complex clause

[choices 형식]
{"number":1,"text":"① which was established"}
→ 원형번호 + 공백 + modified_passage와 완전히 동일한 표현. paraphrase·축약 금지.
→ 5개 choices 표현이 모두 달라야 함.

[오류 변형 예시 — 정답 위치]
whether → that  /  to reduce → reducing  /  themselves → them
which was → which were  /  interesting → interested  /  defend → defending

[출력 필드]
modified_passage / question_text / choices / answer
explanation: 왜 틀렸는지 + 어떤 문법 규칙 위반 + 올바른 형태를 한글로 설명.
`,
