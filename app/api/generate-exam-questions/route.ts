...
Check whether exactly ONE grammatical error exists.

For each option:
1. Explain why it is grammatically correct or incorrect.
2. Verify clause structure.
3. Reject if ambiguity exists.
4. Reject if no actual grammar violation exists.
5. Reject if more than one option is incorrect.
6. Reject if the original passage remained unchanged.
7. Reject if the error is based only on awkward wording or intuition.
8. If validation fails, regenerate the item.
