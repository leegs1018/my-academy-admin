...
문제는 단순 어색함 판단 문제가 아니라,
학생이 문장 구조(clause structure)를 실제로 분석해야만 해결 가능해야 한다.

정답은 반드시 "명확한 문법 규칙 위반"이어야 하며,
직관·자연스러움·의미 흐름만으로 풀 수 없어야 한다.
...
"밑줄 포인트는 단어(vocabulary)가 아니라
문법 구조(structural relation)여야 한다."
...
2.
각 번호는 서로 다른 문법 포인트여야 한다.
같은 유형 반복 금지.

3.
choice의 단어/구는
modified_passage 속 표현과 완전히 동일해야 한다.

4.
본문에 없는 표현 생성 금지.

5.
반드시 단 1개만 문법적으로 틀려야 한다.

6.
나머지 4개는 완전히 문법적으로 맞아야 한다.

7.
기존 원문이 이미 문법적으로 완벽한 경우:
→ 반드시 한 곳을 의도적으로 수정하여 오류 생성할 것.

8.
기존 원문 그대로 가져왔는데
실제 오류가 존재하지 않는 상태로 제출하는 것 절대 금지.
...
→ not only reading but also write
...
[1] 관계사
- which / that / who / where / when / why
- 관계대명사 vs 관계부사
- 선행사와 격 불일치
- 삽입구로 선행사 숨기기

예:
the reason where → why
...
예:
not only working hard but also achieve success
...
고난도에서는 반드시 아래 중 하나를 실제 정답으로 사용할 것:
...
1. ①②③④⑤ 모두 존재하는가?
2. choice와 본문 표현 동일한가?
3. 실제 오류는 정확히 1개인가?
4. 나머지 4개는 완전히 맞는가?
5. 밑줄 포인트가 구조 기반인가?
6. 직관만으로 풀 수 없는가?
7. clause boundary 분석 필요한가?
8. 평가원 스타일 함정인가?
9. 단순 어색함 문제가 아닌가?
10. 원문 그대로 가져와 정답 없는 상태 아닌가?
...
규칙:
- 원본 지문을 기반으로 재구성하되, 자연스러운 수능형 순서 배열 지문으로 작성.
- [주어진 글]은 도입부로서 독립적으로 이해 가능한 1~2문장.
- (A)(B)(C)는 각각 완결된 단락으로 작성.
- 각 단락은 지시어·연결어·대명사를 포함하여 앞 내용 없이는 이해 불가한 구조로 설계.
