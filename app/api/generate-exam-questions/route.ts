...
[핵심 원칙] 반드시 위에 제시된 원본 지문만 사용합니다. 새로운 지문을 창작하거나 원본과 다른 주제의 지문을 사용하는 것은 절대 금지입니다. 원지문의 단어·문장·표현을 절대 변형하지 마세요. 삽입할 문장 제거 및 ①~⑤ 기호 삽입 외에 어떤 수정도 금지합니다.

⚠️ modified_passage는 위에 제시된 원본 지문을 그대로 사용해야 합니다. 새로운 영어 문장을 창작하거나 원본 지문과 다른 내용의 지문을 modified_passage로 작성하는 것은 절대 금지입니다.

생성 규칙:
1. 위에 제시된 원본 지문에서 삽입하기 좋은 문장 1개를 원문 그대로 선택합니다. (접속어/대명사 연결이 자연스러운 문장 우선)
...
[modified_passage 형식]
위 원본 지문에서 선택한 문장을 제거하고 ①~⑤ 기호가 삽입된 지문. 나머지 원문 그대로.
...
5. 핵심 개념 일치 검증
최종 요약문 작성 후 다음 질문에 답하라.
"수험생이 반드시 이해해야 하는 핵심 개념 두 개는 무엇인가?"
그 두 개의 답과 (A)(B)가 일치하지 않는다면, (A)(B)를 재선정하고 요약문을 다시 작성하라.
빈칸은 '가려도 되는 정보'가 아니라 필자의 핵심 메시지를 결정하는 개념이어야 한다.
