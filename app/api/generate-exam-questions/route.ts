...
3. 분사
   ...
   - 분사구문의 논리주어 불일치
   - 감정분사 혼동 (interested vs interesting)
...
5개의 밑줄 포인트는 반드시 서로 다른 문법 카테고리에서 선정할 것:
  (A) 준동사 구조 — to V / Ving / bare V 선택 (it-가목적어, lead to, 지각동사 등)
  (B) 수식·일치 관계 — 주어-동사 수일치 (appositive·관계절로 주어 숨김)
  (C) 관계사·접속사 — 관계대명사/관계부사, what/that/whether
  (D) 분사 구조 — 현재분사·과거분사 형용사, 분사구문 논리주어
...
고난도(C1~C2)에서는 반드시 관계사·병렬구조·분사 중 하나를 실제 정답 오류로 사용할 것.
