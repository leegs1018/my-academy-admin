...
export async function POST(request: Request) {
  try {
    ...
    const { data: items, error: fetchErr } = await adminClient
      .from('mock_workbook_history').select('id, pdf_path, answer_pdf_path')
    ...
    const paths = items.flatMap(i => [i.pdf_path, i.answer_pdf_path].filter(Boolean) as string[]);
    if (paths.length > 0) await adminClient.storage.from('pdf-history').remove(paths);

    const { error: delErr } = await adminClient.from('mock_workbook_history').delete().in('id', items.map(i => i.id));
    ...
    console.error('[delete-mock-workbook-history] 오류:', error);
