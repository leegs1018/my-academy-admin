...
2. 각 문장은 반드시 완전한 문장(대문자 시작, 마침표로 끝)이어야 합니다.
3. 중간에 쉼표로 연결된 절들은 하나의 문장으로 묶습니다.
4. 지문 전체 문장을 모두 포함합니다 (생략 금지). 영어 원문은 절대 변형하지 않습니다.
5. 각 문장에서 핵심 어휘(학생이 알아야 할 중요 단어) 1~3개를 key_words로 지정합니다.
...
9. 오른쪽 해석지 상단 키워드 박스용 내용 압축 불렛을 keyword_bullets 배열로 정확히 2개 작성합니다.
...
출력 형식 (순수 JSON만):
{
  ...
  "keyword_bullets": ["핵심 불렛1 (15~25자 한글)", "핵심 불렛2"],
  "sentences": [
    {
      "en": "원문 영어 문장.",
      "ko": "자연스러운 한국어 해석.",
...
`생성 규칙:
1. 반드시 마침표(.)가 있는 위치에서만 문장을 나눕니다. 쉼표(,), 세미콜론(;), 콜론(:)은 문장 분리 기준이 아닙니다.
2. 각 문장은 반드시 완전한 문장(대문자 시작, 마침표로 끝)이어야 합니다. 중간에 쉼표로 연결된 절들은 하나의 문장으로 묶습니다.
3. 지문 전체 문장을 모두 포함합니다 (생략 금지).
4. "en" 필드는 원문 지문의 해당 문장을 단 한 글자도 바꾸지 않고 그대로 복사합니다. 절대 수정/요약/변형 금지.
