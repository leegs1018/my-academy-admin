...
function buildPrompt(text: string, difficulty: string): string {
  ...
  const tfLengthRule = `각 statement는 반드시 15단어 이상 20단어 이하(word count 기준)로 작성할 것. 15단어 미만 또는 20단어 초과 절대 금지. 종속절(although, while, whereas 등), 분사구문, 관계절, 삽입구 중 하나를 포함하여 문장을 자연스럽게 구성할 것. 작성 후 반드시 단어 수를 세어 15~20단어 범위 내에 있는지 검증할 것. 어휘는 지문 난이도에 맞게 선택.`;
