...
function buildPrompt(text: string, difficulty: string): string {
  ...
  const tfLengthRule = `각 statement는 반드시 영문 기준 130자 이상 160자 이하(character)여야 한다. 130자 미만 또는 160자 초과 절대 금지. 종속절(although, while, whereas, even though 등), 분사구문, 관계절, 삽입구 등을 반드시 포함하여 문장을 충분히 길고 복잡하게 만들 것. 작성 후 반드시 글자 수를 확인하여 130~160자 범위 내에 있는지 검증할 것. 어휘는 지문 난이도에 맞게 선택.`;
